...
setInterval(function () {
    num = currentItem >= 3 ? 0 : currentItem + 1;
    goToItem(num);
}, 10000);
let google_iframe = document.getElementById("elv-maps-frame");
window.addEventListener("scroll", () => {
    let scroll_position = window.scrollY;
    if (scroll_position > 0) {
        if (google_iframe.classList.contains("d-none")) {
            google_iframe.classList.remove("d-none");
        }
    }
});
...
var lengthOfSlide;
// var currentSlide = 0;
// function activateIndicator(i){
//     if(currentSlide < i){
//         var j = i - currentSlide;
//         while(j>0){
//             moveLeftCarousel();
//             j--;
//         }
...
function moveSlidesRight() {
    ...
    slidesArray.forEach(function (el, i) {
        ...
        if (width == 0) {
            ...
            // indicatorArray = indicator.split("-");
            // currentSlide = indicatorArray[1];
        } else {
            indicator = el.querySelector(".invisible-indicator").value;
            if (
                document
                    .getElementById(indicator)
                    .classList.contains("project-active")
            ) {
                document
                    .getElementById(indicator)
...
function moveSlidesLeft() {
    var slides = document.querySelectorAll(".project-carousel-slide");
    var slidesArray = Array.prototype.slice.call(slides);
    slidesArrayreverse = slidesArray.reverse();
    var maxWidth = (slidesArrayreverse.length - 1) * lengthOfSlide;
    slidesArrayreverse.forEach(function (el, i) {
        maxWidth -= lengthOfSlide;
        // maxWidth -= lengthOfSlide*x;
        if (maxWidth == 0) {
            indicator = el.querySelector(".invisible-indicator").value;
            document.getElementById(indicator).classList.add("project-active");
            // indicatorArray = indicator.split("-");
...
var leftNavindicator = document.querySelector(".project-carousel-indicator-left");
leftNavindicator.addEventListener("click", moveRightCarousel);
var rightNavindicator = document.querySelector(".project-carousel-indicator-right");
rightNavindicator.addEventListener("click", moveLeftCarousel);
...
touch_carousel_slider.addEventListener("touchstart", startTouchCarouselSlider, {
    passive: true,
});
touch_carousel_slider.addEventListener("touchmove", moveTouchInfiniteSlider, {
    passive: true,
});
...
// function activateIndicatorReal(i){
//     if(realcurrentSlide < i){
//         var j = i - realcurrentSlide;
//         while(j>0){
//             moveLeftCarousel();
//             j--;
//         }
//     }
//     else if(realcurrentSlide > i){
//         var j = realcurrentSlide - i;
//         while(j>0){
...
function real_addClone() {
    ...
    real_carouselContent.insertBefore(
        lastSlide,
        real_carouselContent.firstChild
    );
}
...
function movereal_slidesRight() {
    ...
    real_slidesArray.forEach(function (el, i) {
        el.style.left = width + "px";
        if (width == 0) {
            indicator = el.querySelector(
                ".real-estate-invisible-indicator"
            ).value;
            document
                .getElementById(indicator)
                .classList.add("real-estate-active");
                // indicatorArray = indicator.split("-");
            // currentSlide = indicatorArray[1];
        } else {
            indicator = el.querySelector(
            ...
            if (
                document
                    .getElementById(indicator)
                    .classList.contains("real-estate-active")
            ) {
                document
                    .getElementById(indicator)
                    .classList.remove("real-estate-active");
            }
        }
...
function getreal_screenSize() {
    ...
    if (real_carouselwidth > 1440) {
        real_carouselwidth = 1440;
    }
...
var realleftNavindicator = document.querySelector(".real-carousel-indicator-left");
realleftNavindicator.addEventListener("click", moveRightreal_carousel);
var realrightNavindicator = document.querySelector(".real-carousel-indicator-right");
realrightNavindicator.addEventListener("click", moveLeftreal_carousel);
var moving = true;
...
let touch_real_carousel_slider = document.getElementById(
    "real-estate-touch-carousel-slider"
);
touch_real_carousel_slider.addEventListener(
    "touchstart",
    startTouchreal_carouselSlider,
    { passive: true }
);
touch_real_carousel_slider.addEventListener(
    "touchmove",
    real_moveTouchInfiniteSlider,
    { passive: true }
);
...
// home Counter scroll datavalue change
let a = 0;
const counter = document.querySelector("#counter_home");
const countElements = document.querySelectorAll(".home-data-count");

const animateCount = (element, countTo, duration) => {
    let countNum = parseInt(element.innerText, 10);
    let start = null;
    const step = (timestamp) => {
        if (!start) start = timestamp;
        const progress = timestamp - start;
        element.innerText = Math.floor((progress / duration) * countTo);
        if (progress < duration) window.requestAnimationFrame(step);
        else element.innerText = countTo;
    };
...
const handleScroll = () => {
    if (a || !isScrolledIntoView(counter)) return;
...
var ventures_carouselContent = document.querySelector(
    ".ventures-carousel-content"
);
...
function ventures_addClone() {
    ...
    ventures_carouselContent.insertBefore(
        lastSlide,
        ventures_carouselContent.firstChild
    );
}
...
function setventures_screenSize() {
    if (window.innerWidth >= 768) {
        ventures_carouselDisplaying = 3.5;
    } else if (window.innerWidth >= 768) {
        ventures_carouselDisplaying = 2.5;
    } else {
...
function getventures_screenSize() {
    ...
    ventures_lengthOfSlide =
        ventures_carouselwidth / ventures_carouselDisplaying;
...
function ventures_moveBasedOnMouse(e) {
    ...
    ventures_carouselContent.removeEventListener(
        "mousemove",
        ventures_slightMove
    );
}

let touch_ventures_carousel_slider = document.getElementById(
    "ventures-touch-carousel-slider"
);
touch_ventures_carousel_slider.addEventListener(
    "touchstart",
    startTouchventures_carouselSlider,
    { passive: true }
);
touch_ventures_carousel_slider.addEventListener(
    "touchmove",
    ventures_moveTouchInfiniteSlider,
    { passive: true }
);
...
function ventures_moveTouchInfiniteSlider(e) {
    ...
    var diffventures_carouselX =
        initialventures_carouselX - currentventures_carouselX;
    var diffventures_carouselY =
        initialventures_carouselX - currentventures_carouselY;
...
function client_addClone() {
    ...
    client_carouselContent.insertBefore(
        lastSlide,
        client_carouselContent.firstChild
    );
}
    ...
    client_slidesArray.forEach(function (el, i) {
        ...
        if (width == 0) {
            ...
            if (
                document
                    .getElementById(indicator)
                    .classList.contains("client-active")
            ) {
                document
                    .getElementById(indicator)
                    .classList.remove("client-active");
            }
        }
...
var clientleftNavindicator = document.querySelector(".client-carousel-indicator-left");
clientleftNavindicator.addEventListener("click", moveRightclient_carousel);
var clientrightNavindicator = document.querySelector(".client-carousel-indicator-right");
clientrightNavindicator.addEventListener("click", moveLeftclient_carousel);
var moving = true;
...
let touch_client_carousel_slider = document.getElementById(
    "client-touch-carousel-slider"
);
touch_client_carousel_slider.addEventListener(
    "touchstart",
    startTouchclient_carouselSlider,
    { passive: true }
);
touch_client_carousel_slider.addEventListener(
    "touchmove",
    client_moveTouchInfiniteSlider,
    { passive: true }
);
...
function client_moveTouchInfiniteSlider(e) {
    ...
    var diffclient_carouselX =
        initialclient_carouselX - currentclient_carouselX;
    var diffclient_carouselY =
        initialclient_carouselX - currentclient_carouselY;
...
// home location address change
function map_change(id) {
    let google_iframe_loc1 = document.getElementById("elv-maps-frame");
    let elv_homelocation = "";
    switch (id) {
        case 1:
            elv_homelocation = `https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d15552.509447133234!2d77.7351823!3d12.9637011!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae12013c8bcc11%3A0x1cfc39a328fdadf7!2sELV%20KINGSLAND!5e0!3m2!1sen!2sin!4v1679654010480!5m2!1sen!2sin`;
            break;
        case 2:
            elv_homelocation = `https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d62214.41601885157!2d77.6500025!3d12.9461727!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae133a12308573%3A0x2c5b1937223e1e0f!2sELV%20Brindavan%20Layout!5e0!3m2!1sen!2sin!4v1679654121108!5m2!1sen!2sin`;
            break;
        case 3:
            elv_homelocation = `https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d15552.509447133234!2d77.7351823!3d12.9637011!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae0dff330bed8b%3A0x8c83a8d377a391a4!2sELV%20HIGHGARDEN!5e0!3m2!1sen!2sin!4v1679653948570!5m2!1sen!2sin`;
            break;
        case 4:
            elv_homelocation = `https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d62202.492798248844!2d77.6968017!3d12.9938532!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae0de280d6a4cf%3A0x636ffcdb76481e04!2sELV%20Signature!5e0!3m2!1sen!2sin!4v1679654142232!5m2!1sen!2sin`;
            break;
        case 5:
            elv_homelocation = `https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d15552.509447133234!2d77.7351823!3d12.9637011!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae13bfc2e0145f%3A0xd17e21a05a9b604!2sELV%20Marvel!5e0!3m2!1sen!2sin!4v1679654036066!5m2!1sen!2sin`;
            break;
        case 6:
            elv_homelocation = `https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d62247.75851213995!2d77.7112112!3d12.8119116!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3bae7334be74db85%3A0x32d5a1b836db8290!2sELV%20Akruti%20Ark!5e0!3m2!1sen!2sin!4v1679654068019!5m2!1sen!2sin`;
            break;
        default:
            break;
    }
    google_iframe_loc1.src = elv_homelocation;
}
// home location address change
...
function people_addClone() {
    ...
    people_carouselContent.insertBefore(
        lastSlide,
        people_carouselContent.firstChild
    );
}
    ...
    people_slidesArray.forEach(function (el, i) {
        ...
        if (width == 0) {
            ...
            if (
                document
                    .getElementById(indicator)
                    .classList.contains("people-active")
            ) {
                document
                    .getElementById(indicator)
                    .classList.remove("people-active");
            }
        }
...
var peopleleftNavindicator = document.querySelector(".people-carousel-indicator-left");
peopleleftNavindicator.addEventListener("click", moveRightpeople_carousel);
var peoplerightNavindicator = document.querySelector(".people-carousel-indicator-right");
peoplerightNavindicator.addEventListener("click", moveLeftpeople_carousel);
var moving = true;
...
let touch_people_carousel_slider = document.getElementById(
    "people-touch-carousel-slider"
);
touch_people_carousel_slider.addEventListener(
    "touchstart",
    startTouchpeople_carouselSlider,
    { passive: true }
);
touch_people_carousel_slider.addEventListener(
    "touchmove",
    people_moveTouchInfiniteSlider,
    { passive: true }
);
...
function people_moveTouchInfiniteSlider(e) {
    ...
    var diffpeople_carouselX =
        initialpeople_carouselX - currentpeople_carouselX;
    var diffpeople_carouselY =
        initialpeople_carouselX - currentpeople_carouselY;
