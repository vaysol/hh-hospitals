...
<div class="container row">
    <div>
    <?php if ($project_highlights){foreach($project_highlights as $data){} ?>
        <h2>Edit Project Highlights</h2>
        <?php }else{ ?>
        <h2>Add Project Highlights</h2>
        <?php } ?>
    </div>
    <div class="col-md-8">
        <form action="<?= base_url()?>/admin/projects/highlights/save/" class="was-validated1 row" method="POST"
            enctype="multipart/form-data">
            <div class="mb-3 col-md-6" hidden>
                <label for="id" class="form-label">Highlights Id</label>
                <input value="<?php if (!empty($data['id'])){ echo $data['id']; } ?>" type="text" class="form-control"
                    id="id" placeholder="Highlights Id" name="id">
            </div>
            ...
            <div class="mb-3 col-md-6">
                ...
                <select class="form-select" aria-label="Select Project" name="project_id" required>
                    <option class="text-muted" <?php if (empty($data['project_id'])){ echo "selected"; } ?> value=""
                        disabled>Please Select..
                    </option>
                    <option class="text-success" <?php if ($data['project_id']==1){ echo "selected"; } ?> value="1">
                        Project 1</option>
                    <option class="text-danger" <?php if ($data['project_id']==0){ echo "selected"; }  ?> value="0">
                        Project 2</option>
                </select>
                <div class="invalid-feedback">Please fill out this field.</div>
            </div>
    ...
    <?php if ($project_highlights){ ?>
    <div class="col-md-4">
        <p>Desktop Image :</p>
        <img id="desktop_image_preview"
            src="<?= base_url()?>/assets/images/project_highlights/desktop/<?php echo $data['desktop_image']; ?>"
            style="width: 100%;height: auto;object-fit: cover;">
        <p></p>
        <p>Mobile Image :</p>
        <img id="mobile_image_preview"
            src="<?= base_url()?>/assets/images/project_highlights/mobile/<?php echo $data['mobile_image']; ?>"
            style="width: 100%;height: auto;object-fit: cover;">
    </div>
    <?php }else{ ?>
    <div class="col-md-4">
