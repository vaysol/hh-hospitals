<div class="container row">
    <div>
        <?php if ($project_amenities){foreach($project_amenities as $data){} ?>
            <h2>Edit Project Amenities</h2>
            <?php }else{ ?>
            <h2>Add Project Amenities</h2>
        <?php } ?>
    </div>
    <div class="col-md-8">
        <form action="<?= base_url()?>/admin/projects/amenities/save/" class="was-validated1 row" method="POST"
            enctype="multipart/form-data">
            <div class="mb-3 col-md-6" hidden>
                <label for="id" class="form-label">ProjectAmenities Id</label>
                <input value="<?php if (!empty($data['id'])){ echo $data['id']; } ?>" type="text" class="form-control"
                    id="id" placeholder="ProjectAmenities Id" name="id">
            </div>
    ...
    <div class="col-md-4">
        ...
        <img id="image_preview"
            src="<?= base_url()?>/<?php echo $data['image']; ?>.webp"
