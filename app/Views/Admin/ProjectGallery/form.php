<div class="container row">
    ...
    <?php if ($gallery){foreach($gallery as $data){} ?>
        <h2>Edit Project Gallery</h2>
        <?php }else{ ?>
        <h2>Add Project Gallery</h2>
        <?php } ?>
    </div>
    <div class="col-md-8">
        <form action="<?= base_url()?>/admin/projects/gallery/save/" class="was-validated1 row" method="POST"
            ...
            <div class="mb-3 col-md-6">
                ...
                <select class="selectpicker col-12" multiple aria-label="size 3 select example" id="project_id"
                    placeholder="Select Project" name="project_id[]" >
                    <?php if ($all_projects){foreach($all_projects as $all_projects){ ?>
                    <option value="<?php echo $all_projects['id']; ?>"
                        <?php if(!empty($data['project_id'])){if(match_selected($all_projects['id'],$data['project_id'])){echo "selected";}}?>>
    ...
    <div class="col-md-4">
        ...
        <img id="desktop_image_preview"
            src="<?= base_url()?>/assets/images/project/gallery/desktop/<?php echo $data['desktop_image']; ?>"
            style="width: 100%;height: auto;object-fit: cover;">
        <p></p>
        <p>Mobile Image :</p>
        <img id="mobile_image_preview"
            src="<?= base_url()?>/assets/images/project/gallery/mobile/<?php echo $data['mobile_image']; ?>"
            style="width: 100%;height: auto;object-fit: cover;">
    </div>
    <?php }else{ ?>
    <div class="col-md-4">
