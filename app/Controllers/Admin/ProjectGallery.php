...
use App\Models\Admin\ProjectGallery_Model;

class ProjectGallery extends Controller {
    public $db, $session, $model;

    function __construct() {
        helper('custom');
        $this->model = new ProjectGallery_Model();
    ...
    public function index() {
        ...
        echo view( 'Admin/ProjectGallery/index' );
    ...
    public function add() {
        ...
        echo view( 'Admin/header', $data );
        echo view( 'Admin/ProjectGallery/form' );
        echo view( 'Admin/footer' );
    }
    ...
    public function edit( $id ) {
        ...
        $data[ 'gallery' ] = $this->model->getGalleryById( $id );
        $data[ 'all_projects' ] = $this->model->getProjects();
    ...
    public function save() {
        ...
        if ( empty( $this->request->getPost( 'id' ) ) ) {
            ...
            if ( $this->request->getFile( 'desktop_image' ) ) {
                ...
                $imageName = uploadFile( $image, 'assets/images/project/gallery/desktop/' );
                $data[ 'desktop_image' ] = $imageName != NULL ? $imageName : '';
            }
            if ( $this->request->getFile( 'mobile_image' ) ) {
                $image = $this->request->getFile( 'mobile_image' );
                $imageName = uploadFile( $image, 'assets/images/project/gallery/mobile/' );
            ...
            $isUpdated = $this->model->updateGallery( $data, $galleryId );
            if ( $isUpdated ) {
                $msg = array( 'type' => 'success', 'icon' => 'icon-ok green', 'txt' => 'Save Changes Updated Successfully' );
                session()->setFlashdata( 'msg', $msg );
                return redirect()->to( '/admin/projects/gallery/' );
