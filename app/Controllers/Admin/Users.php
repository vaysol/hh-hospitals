...
class Users extends Controller {
    ...
    public function edit( $id ) {
        ...
        $data[ 'users' ] = $this->model->getUserById( $id );
        echo view( 'Admin/header', $data );
        echo view( 'Admin/Users/form' );
        echo view( 'Admin/footer' );
    }
    ...
    public function save() {
        ...
        if ( empty( $this->request->getPost( 'user_id' ) ) ) {
            ...
            if ( $this->request->getFile( 'image' ) ) {
                ...
                $imageName = uploadFile( $image, 'assets/images/user/' );
            ...
            $isInsert = $this->model->insertUser( $data );
            if ( $isInsert ) {
            ...
            $isUpdated = $this->model->updateUser( $data, $userId );
            if ( $isUpdated ) {
                $msg = array( 'type' => 'success', 'icon' => 'icon-ok green', 'txt' => 'Save Changes Updated Successfully' );
                session()->setFlashdata( 'msg', $msg );
                return redirect()->to( '/admin/users/' );
    ...
    public function delete( $id ) {
        $query = $this->model->deleteUser( $id );
        if ( $query ) {
